...
const userRouter = async (request, response) => {
  if (
    ...
    try {
      ...
      response.end(JSON.stringify({ status: 200, message: "Success, use token for authorization", token: userData }, null, 5));
    } catch (err) {
      response.writeHead(404, { "Content-Type": "application/json" });
      response.end(JSON.stringify({ status: 404, message: err }, null, 5));
    }
  } else if (request.url.match(/api\/user\/confirm\/eyJ[A-Za-z0-9-_]+\.eyJ[A-Za-z0-9-_]+\.[A-Za-z0-9-_.+/]*/g) && request.method == "GET") {
