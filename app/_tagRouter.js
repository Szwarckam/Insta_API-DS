...
const tagRouter = async (request, response) => {
  if (request.url.match(/\/api\/tags\/raw$/) && request.method == "GET") {
    ...
    try {
      ...
      response.writeHead(200, { "Content-Type": "application/json" });
      response.end(JSON.stringify({ status: 200, length: allConvertedTags.length, tags: allConvertedTags }, null, 5));
    } catch (err) {
      response.writeHead(404, { "Content-Type": "application/json" });
      response.end(JSON.stringify({ status: 404, message: err }, null, 5));
    }
    ...
    if (tagData.name.match(/\#(\w+)$/)) {
      ...
      response.writeHead(404, { "Content-Type": "application/json" });
      response.end(JSON.stringify({ status: 404, message: `Invalid tag name, must contains #` }, null, 5));
    }
